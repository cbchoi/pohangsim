...
from UI.building_dialog import BuildingTypeManager
from UI.control_box import controlBox
from UI.scenario_editor import ScenarioClass, new_scenario_GUI, load_scenario_GUI, save_scenario_GUI
# matplot
...
class ScenarioListManager(QDialog):
    ...
    def edit_scenario(self):
        if self.listWidget.currentRow()>=0:
            ...
            ui_file = QFile("UI/screen/BuildingDialog.ui")
            loader = QUiLoader()
...
class OutputManager(QObject):
    ...
    def show_result(self, Ncan,fileurl):
        ...
        if fileurl.startswith("output"):
            ...
            try:
                log = pd.read_csv("output/"+fileurl.split("/")[1] + ".log", index_col=False, names=['1', '2', '3', '4', '5', '6'])
                df = pd.DataFrame(index=range(0, 3), columns=['job', 'reports'])
                df['job'] = log['1'][0], log['3'][0], log['5'][0]
                df['reports'] = log['2'][0], log['4'][0], log['6'][0]
                df=df.dropna(axis=0)
