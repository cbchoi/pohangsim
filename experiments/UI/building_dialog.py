...
class BuildingTypeManager(QDialog):
	...
	def add_building(self):
		self.scenario.add(BuildingClass())
		self.update_page()
	def remove_buildings(self):
		index_start = 6 * (self.currentpage - 1)
		if self.B1.isChecked():
			self.scenario.remove(self.scenario[index_start])
		if self.B2.isChecked():
			self.scenario.remove(self.scenario[index_start+1])
		if self.B3.isChecked():
			self.scenario.remove(self.scenario[index_start+2])
		if self.B1_2.isChecked():
			self.scenario.remove(self.scenario[index_start+3])
		if self.B2_2.isChecked():
			self.scenario.remove(self.scenario[index_start+4])
		if self.B3_2.isChecked():
			self.scenario.remove(self.scenario[index_start+5])
		self.update_page()
	...
	def update_page(self):
		...
		self.update_text(index_start,*showlist[index_start:index_end])
